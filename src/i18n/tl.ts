export default {
  // Site
  "site.title": "Ang Proyekto para sa Pangkalahatang Kapakanan",
  "site.tagline": "Alamin ang iyong mga karapatan. Maghanap ng tulong.",
  // Common
  "common.home": "Bahay",
  "common.knowYourRights": "Alamin ang Iyong mga Karapatan",
  "common.getHelp": "Humingi ng Tulong",
  "common.callNow": "Tumawag Ngayon",
  // Housing
  "housing.title": "Pabahay",
  "housing.cardDesc":
    "Mga karapatan ng nangungupahan, depensa sa pagpapaalis, tulong sa pabahay",
  "housing.pageDesc":
    "Mga karapatan ng nangungupahan, depensa sa pagpapaalis, at mga programa ng tulong sa pabahay.",
  "housing.tenantRights": "Alamin ang Hindi Magagawa ng Iyong Kasero",
  "housing.tenantRightsDesc":
    "Mayroon kang mga legal na karapatan sa isang ligtas na tahanan, pagbabalik ng iyong perang deposito sa seguridad, at isang patas at tapat na pag-upa. Ipapaliwanag nito kung ano ang HINDI pinapayagang gawin ng mga kasero.",
  "housing.eviction": "Maaari Mong Hamunin ang Isang Pagpapaalis",
  "housing.evictionDesc":
    "Hindi mo kailangang umalis kaagad. Mayroon kang mga legal na karapatan sa panahon ng proseso ng pagpapaalis --- at mga libreng abogado na makakatulong sa iyo na hamunin ito.",
  "housing.assistance": "Mag-aplay para sa Tulong sa Pag-upa",
  "housing.assistanceDesc":
    "Maaari kang maging kwalipikado para sa tulong sa pagbabayad ng upa ngayon. Mag-aplay para sa tulong sa pag-upa para sa emergency, Seksyon 8, o pampublikong pabahay sa pamamagitan ng iyong lokal na awtoridad sa pabahay.",
  "housing.legalAid": "Libreng Tulong Legal para sa mga Isyu sa Pabahay",
  "housing.legalAidDesc":
    "Makakakuha ka ng libreng abogado para sa depensa sa pagpapaalis, mga hindi ligtas na kondisyon, o mga hindi pagkakaunawaan sa may-ari ng lupa. Libre ang tulong legal at makukuha sa bawat estado.",
  "housing.phone": "211",
  "housing.phoneDesc":
    "Mga rekomendasyon para sa lokal na tulong sa pabahay at mga serbisyong panlipunan",
  // Voting
  "voting.title": "Pagboto",
  "voting.cardDesc":
    "Pagpaparehistro, impormasyon sa botohan, ID ng botante, proteksyon sa halalan",
  "voting.pageDesc":
    "Pagpaparehistro, impormasyon sa botohan, mga batas sa ID ng botante, at proteksyon sa halalan.",
  "voting.registration": "Magparehistro para Bumoto",
  "voting.registrationDesc":
    "Tumatagal ito ng humigit-kumulang 2 minuto. Suriin ang katayuan ng iyong pagpaparehistro, at kung kinakailangan, magparehistro online dito.",
  "voting.voterId": "Suriin Kung Anong ID ang Kailangan Mo para Bumoto",
  "voting.voterIdDesc":
    "Iba-iba ang bawat estado. Hanapin kung anong ID ang kinakailangan ng iyong estado --- ang ilang estado ay hindi nangangailangan ng kahit ano.",
  "voting.pollingPlace": "Hanapin ang Iyong Lugar ng Botohan",
  "voting.pollingPlaceDesc":
    "Ilagay ang iyong address at kunin ang eksaktong lokasyon ng botohan, oras, at kung ano ang nasa iyong balota.",
  "voting.electionProtection": "Iulat ang Pananakot o Problema ng Botante",
  "voting.electionProtectionDesc":
    "Kung may sumubok na pigilan ka sa pagboto, ito ay isang pederal na krimen. Maaari mo itong iulat at makakuha ng agarang tulong mula sa mga sinanay na legal na boluntaryo.",
  "voting.phone": "866-OUR-VOTE (866-687-8683)",
  "voting.phoneDesc":
    "Hotline para sa Proteksyon sa Halalan --- tulong sa botanteng walang kinikilingan",
  // LGBTQ+
  "lgbtq.title": "Mga Karapatan ng LGBTQ+",
  "lgbtq.cardDesc":
    "Mga organisasyong sumusuporta laban sa diskriminasyon, mga karapatan ng trans,",
  "lgbtq.pageDesc":
    "Mga proteksyon laban sa diskriminasyon, mga karapatan ng trans, at mga organisasyong sumusuporta.",
  "lgbtq.antiDiscrimination":
    "Ilegal ang Diskriminasyon Batay sa Pagkakakilanlan",
  "lgbtq.antiDiscriminationDesc":
    "Ipinagbabawal ng batas pederal ang diskriminasyon sa trabaho, sa pabahay, at sa mga pampublikong lugar batay sa kung sino ka. Kung nakaranas ka ng diskriminasyon, maaari kang magsampa ng kaso dito.",
  "lgbtq.transRights": "Pag-update ng Iyong Pangalan at Mga Dokumento ng ID",
  "lgbtq.transRightsDesc":
    "Mga sunud-sunod na gabay sa pagpapalit ng iyong pangalan, marker ng kasarian sa mga ID, at pag-access sa pangangalagang pangkalusugan --- para sa bawat estado.",
  "lgbtq.legalSupport": "May Libreng Tulong Legal na Magagamit",
  "lgbtq.legalSupportDesc":
    "Libreng tumatanggap ng mga kaso ang help desk ng Lambda Legal. Kung ikaw ay nadiskrimina, natanggal sa trabaho, pinagkaitan ng pabahay, o pinagkaitan ng pangangalagang pangkalusugan dahil sa kung sino ka --- maaari mo silang kontakin para sa tulong.",
  "lgbtq.crisisResources": "Makipag-usap sa Isang Taong Nakakaintindi",
  "lgbtq.crisisResourcesDesc":
    "Kumpidensyal na suporta mula sa mga taong nakakaintindi. Makipag-chat, mag-text, o tumawag --- available sa sinumang LGBTQ+ na tao, hindi lamang sa mga sitwasyon ng krisis.",
  "lgbtq.phone988": "988",
  "lgbtq.phone988Desc":
    "Suicide & Crisis Lifeline --- pindutin ang 3 para sa suporta ng LGBTQ+",
  "lgbtq.phoneTrevor": "866-488-7386",
  "lgbtq.phoneTrevorDesc":
    "The Trevor Project --- suporta sa krisis para sa mga kabataang LGBTQ+",
  // Free Speech
  "freeSpeech.title": "Malayang Pagsasalita",
  "freeSpeech.cardDesc":
    "Unang Susog, mga karapatan sa protesta, kalayaan sa pamamahayag",
  "freeSpeech.pageDesc":
    "Mga karapatan sa Unang Susog, mga karapatan sa protesta, at kalayaan sa pamamahayag.",
  "freeSpeech.firstAmendment":
    "Hindi Maaring Limitahan ng Gobyerno ang Iyong Pagsasalita",
  "freeSpeech.firstAmendmentDesc":
    "Pinoprotektahan ng Unang Susog ang iyong karapatang magsalita, maglathala, sumamba, magtipon, at magpetisyon sa gobyerno. Ipapaliwanag nito kung ano ang ibig sabihin nito sa pagsasagawa.",
  "freeSpeech.protest": "Ang Iyong Karapatan na Magprotesta",
  "freeSpeech.protestDesc":
    "Maaari kang magprotesta sa mga pampublikong bangketa at sa mga parke nang walang permit. Saklaw nito ang mga magagawa at hindi magagawa ng pulisya, at ang gagawin kung nilabag ang iyong mga karapatan.",
  "freeSpeech.legalSupport":
    "Kung Pinaghigpitan ang Iyong Pagsasalita, Maaaring Tumulong ang ACLU",
  "freeSpeech.legalSupportDesc":
    "Kung pinarusahan ka ng gobyerno dahil sa pagsasalita, pagprotesta, o paglalathala, maaari kang magkaroon ng kaso sa Unang Susog. Isumite ang iyong sitwasyon para sa pagsusuri.",
  "freeSpeech.knowRightsProtest":
    "Ano ang Gagawin Kung Arestuhin sa Isang Protesta",
  "freeSpeech.knowRightsProtestDesc":
    "Huwag magsalita ng kahit ano maliban sa \u2018Gusto ko ng abogado.\u2019 Saklaw ng gabay na ito ang gagawin kung ikaw ay arestuhin, ikulong, o ang iyong telepono ay kinumpiska sa isang protesta.",
  "freeSpeech.phone": "212-679-5100",
  "freeSpeech.phoneDesc":
    "National Lawyers Guild --- legal na tulong sa malayang pananalita at mga kalayaang sibil",
  // Workers' Rights
  "workers.title": "Mga Karapatan ng mga Manggagawa",
  "workers.cardDesc":
    "Pagnanakaw ng sahod, kaligtasan sa lugar ng trabaho, labor board, kawalan ng trabaho",
  "workers.pageDesc":
    "Pagnanakaw ng sahod, kaligtasan sa lugar ng trabaho, mga reklamo sa labor board, at insurance sa kawalan ng trabaho.",
  "workers.wageHour": "May Karapatan Ka sa Bawat Dolyar na Kinita Mo",
  "workers.wageHourDesc":
    "Ang pagnanakaw ng sahod ang pinakakaraniwang uri ng pagnanakaw sa Amerika. Kung hindi ka binayaran ng minimum na sahod, overtime, o kinuha ang iyong mga tip --- pinoprotektahan ka ng batas.",
  "workers.safety":
    "Maaari Mong Iulat ang mga Hindi Ligtas na Kondisyon Nang Kumpidensyal",
  "workers.safetyDesc":
    "Pinoprotektahan ng OSHA ang mga manggagawa. Maaari kang maghain ng kumpidensyal na reklamo tungkol sa mga hindi ligtas na kondisyon, at ang iyong employer ay hindi maaaring legal na gumanti.",
  "workers.complaint":
    "Maghain ng Reklamo sa Pagnanakaw ng Sahod --- Walang Gastos",
  "workers.complaintDesc":
    "Iimbestigahan ng Kagawaran ng Paggawa ang iyong employer nang walang bayad. Hindi mo kailangan ng abogado. Maghain online ngayon.",
  "workers.unemployment":
    "Mag-apply para sa Mga Benepisyo sa Kawalan ng Trabaho",
  "workers.unemploymentDesc":
    "Kung nawalan ka ng trabaho, malamang na kwalipikado ka. Ang bawat estado ay may online na aplikasyon. Nagbayad ka na sa sistemang ito --- ito ang mga benepisyong iyong nakuha.",
  "workers.phone": "1-866-4-USWAGE (1-866-487-2365)",
  "workers.phoneDesc": "Kagawaran ng Paggawa --- mga paglabag sa sahod at oras",
  // Food
  "food.title": "Pagkain",
  "food.cardDesc":
    "Mga food stamp, food bank, pagkain sa paaralan, hotline para sa kagutuman",
  "food.pageDesc":
    "Tulong sa pagkain, mga benepisyo ng SNAP, food bank, at libreng pagkain sa paaralan.",
  "food.snapEligibility":
    "Maaari Ka Nang Maging Kwalipikado para sa Tulong sa Pagkain",
  "food.snapEligibilityDesc":
    "Milyun-milyong Amerikano ang kwalipikado para sa SNAP (food stamp) ngunit hindi kailanman nag-aaplay. Kung nahihirapan kang bumili ng mga groceries, maaari kang maging karapat-dapat para sa mga buwanang benepisyo --- anuman ang iyong edad o laki ng pamilya.",
  "food.schoolMeals":
    "Maaaring Kumain nang Libre ang Iyong mga Anak sa Paaralan",
  "food.schoolMealsDesc":
    "Kung mababa ang kita ng iyong pamilya, maaaring makakuha ng libreng almusal at tanghalian ang iyong mga anak tuwing araw ng pasukan. Ito ay isang pederal na programa --- bawat pampublikong paaralan sa Amerika ay nakikilahok.",
  "food.foodBank": "Maghanap ng Libreng Pagkain Malapit sa Iyo",
  "food.foodBankDesc":
    "Ang mga food bank ay nagbibigay ng libreng mga groceries --- walang ID, walang papeles, walang mga tanong. Ilagay ang iyong zip code upang makahanap ng isa na malapit sa iyo ngayon.",
  "food.snapApply": "Mag-apply para sa SNAP (Food Stamps)",
  "food.snapApplyDesc":
    "Naglalagay ang SNAP ng pera sa isang card bawat buwan para sa mga grocery. Maaari kang mag-apply online sa pamamagitan ng iyong estado. Kung kwalipikado ka, maaaring magsimula ang mga benepisyo sa loob ng 30 araw.",
  "food.phone": "1-866-3-HUNGRY (1-866-348-6479)",
  "food.phoneDesc":
    "USDA National Hunger Hotline --- libreng tulong sa paghahanap ng pagkain malapit sa iyo",
  // Donate
  "donate.title": "Mag-donate",
  "donate.message":
    "Mag-donate sa The General Welfare Project sa pamamagitan ng pagboto sa halalan sa kalagitnaan ng termino sa 2026",
} as Record<string, string>;
