export default {
  // Site
  "site.title": "The General Welfare Project",
  "site.tagline": "Conozca sus derechos. Encuentre ayuda.",
  // Common
  "common.home": "Inicio",
  "common.knowYourRights": "Conozca sus derechos",
  "common.getHelp": "Obtenga ayuda",
  "common.callNow": "Llame ahora",
  // Housing
  "housing.title": "Vivienda",
  "housing.cardDesc":
    "Derechos de los inquilinos, defensa contra el desalojo, asistencia para la vivienda",
  "housing.pageDesc":
    "Derechos de los inquilinos, defensa contra el desalojo y programas de asistencia para la vivienda.",
  "housing.tenantRights": "Sepa lo que su arrendador no puede hacer",
  "housing.tenantRightsDesc":
    "Usted tiene derechos legales a una vivienda segura, a la devolución de su depósito de seguridad y a un contrato de arrendamiento justo y honesto. Aquí se explica lo que los arrendadores NO tienen permitido hacer.",
  "housing.eviction": "Puede impugnar un desalojo",
  "housing.evictionDesc":
    "No tiene que irse de inmediato. Usted tiene derechos legales durante el proceso de desalojo, y abogados gratuitos que pueden ayudarle a impugnarlo.",
  "housing.assistance": "Solicite asistencia para el alquiler",
  "housing.assistanceDesc":
    "Es posible que califique para recibir ayuda con el pago del alquiler en este momento. Solicite asistencia de emergencia para el alquiler, el programa de la Sección 8 o vivienda pública a través de su autoridad local de vivienda.",
  "housing.legalAid": "Ayuda legal gratuita para problemas de vivienda",
  "housing.legalAidDesc":
    "Puede obtener un abogado gratuito para defensa contra el desalojo, condiciones inseguras o disputas con el arrendador. La asistencia legal es gratuita y está disponible en todos los estados.",
  "housing.phone": "211",
  "housing.phoneDesc":
    "Asistencia local para la vivienda y referencias a servicios sociales",
  // Voting
  "voting.title": "Votación",
  "voting.cardDesc":
    "Registro, información sobre centros de votación, identificación de votante, protección electoral",
  "voting.pageDesc":
    "Registro, información sobre centros de votación, leyes de identificación de votante y protección electoral.",
  "voting.registration": "Regístrese para votar",
  "voting.registrationDesc":
    "Toma unos 2 minutos. Verifique el estado de su registro y, si es necesario, regístrese en línea aquí.",
  "voting.voterId": "Verifique qué identificación necesita para votar",
  "voting.voterIdDesc":
    "Cada estado es diferente. Averigüe exactamente qué identificación requiere su estado; algunos estados no requieren ninguna en absoluto.",
  "voting.pollingPlace": "Encuentre su centro de votación",
  "voting.pollingPlaceDesc":
    "Ingrese su dirección para obtener la ubicación exacta de su centro de votación, los horarios y lo que aparecerá en su boleta electoral.",
  "voting.electionProtection": "Reporte intimidación a votantes o problemas",
  "voting.electionProtectionDesc":
    "Si alguien intenta impedirle votar, es un delito federal. Puede reportarlo y recibir ayuda inmediata de voluntarios legales capacitados.",
  "voting.phone": "866-OUR-VOTE (866-687-8683)",
  "voting.phoneDesc":
    "Línea directa de Protección Electoral --- asistencia electoral no partidista",
  // LGBTQ+
  "lgbtq.title": "Derechos LGBTQ+",
  "lgbtq.cardDesc":
    "Antidiscriminación, derechos de las personas trans, organizaciones de apoyo",
  "lgbtq.pageDesc":
    "Protecciones contra la discriminación, derechos de las personas trans y organizaciones de apoyo.",
  "lgbtq.antiDiscrimination":
    "La discriminación basada en la identidad es ilegal",
  "lgbtq.antiDiscriminationDesc":
    "La ley federal prohíbe la discriminación en el trabajo, la vivienda y los espacios públicos basándose en quién es usted. Si ha sufrido discriminación, puede presentar una denuncia aquí.",
  "lgbtq.transRights": "Actualización de su nombre y documentos de identidad",
  "lgbtq.transRightsDesc":
    "Guías paso a paso para cambiar su nombre y el marcador de género en sus documentos de identidad, y para acceder a la atención médica --- para cada estado.",
  "lgbtq.legalSupport": "Hay ayuda legal gratuita disponible",
  "lgbtq.legalSupportDesc":
    "El servicio de asistencia de Lambda Legal acepta casos de forma gratuita. Si ha sido discriminado, despedido, o se le ha negado vivienda o atención médica debido a quién es usted, puede contactarlos para recibir asistencia.",
  "lgbtq.crisisResources": "Hable con alguien que le entienda",
  "lgbtq.crisisResourcesDesc":
    "Apoyo confidencial entre pares, brindado por personas que le entienden. Chatee, envíe mensajes de texto o llame; el servicio está disponible para cualquier persona LGBTQ+, no solo en situaciones de crisis.",
  "lgbtq.phone988": "988",
  "lgbtq.phone988Desc":
    "Línea de Vida para la Prevención del Suicidio y Crisis --- presione 3 para recibir apoyo LGBTQ+",
  "lgbtq.phoneTrevor": "866-488-7386",
  "lgbtq.phoneTrevorDesc":
    "The Trevor Project --- apoyo en crisis para jóvenes LGBTQ+",
  // Free Speech
  "freeSpeech.title": "Libertad de expresión",
  "freeSpeech.cardDesc":
    "Primera Enmienda, derechos de protesta, libertad de prensa",
  "freeSpeech.pageDesc":
    "Derechos de la Primera Enmienda, derechos de protesta y libertad de prensa.",
  "freeSpeech.firstAmendment":
    "El gobierno no puede restringir su libertad de expresión",
  "freeSpeech.firstAmendmentDesc":
    "La Primera Enmienda protege su derecho a hablar, publicar, practicar su religión, reunirse y presentar peticiones al gobierno. Aquí se explica qué significa esto en la práctica.",
  "freeSpeech.protest": "Su derecho a protestar",
  "freeSpeech.protestDesc":
    "Usted puede protestar en las aceras públicas y en los parques sin necesidad de un permiso. Esta sección cubre lo que la policía puede y no puede hacer, y qué medidas tomar si se violan sus derechos.",
  "freeSpeech.legalSupport":
    "Si se restringió su libertad de expresión, la ACLU podría ayudarle",
  "freeSpeech.legalSupportDesc":
    "Si el gobierno lo ha castigado por hablar, protestar o publicar, es posible que tenga un caso amparado por la Primera Enmienda. Envíe su caso para su revisión.",
  "freeSpeech.knowRightsProtest":
    "Qué hacer si es arrestado durante una protesta",
  "freeSpeech.knowRightsProtestDesc":
    "No diga nada, excepto: «Quiero un abogado». Esta guía explica qué hacer si es arrestado, detenido o si le confiscan el teléfono durante una protesta.",
  "freeSpeech.phone": "212-679-5100",
  "freeSpeech.phoneDesc":
    "National Lawyers Guild --- Asistencia legal en materia de libertad de expresión y libertades civiles",
  // Workers' Rights
  "workers.title": "Derechos de los trabajadores",
  "workers.cardDesc":
    "Robo de salarios, seguridad laboral, junta laboral, desempleo",
  "workers.pageDesc":
    "Robo de salarios, seguridad laboral, quejas ante la junta laboral y seguro de desempleo.",
  "workers.wageHour": "Usted tiene derecho a cada dólar que ha ganado",
  "workers.wageHourDesc":
    "El robo de salarios es la forma más común de robo en Estados Unidos. Si no le pagaron el salario mínimo o las horas extras, o si le retuvieron sus propinas, la ley lo protege.",
  "workers.safety":
    "Puede denunciar condiciones inseguras de forma confidencial",
  "workers.safetyDesc":
    "OSHA protege a los trabajadores. Usted puede presentar una queja confidencial sobre condiciones inseguras, y su empleador no puede tomar represalias legalmente.",
  "workers.complaint":
    "Presente una queja por robo de salarios: sin costo alguno",
  "workers.complaintDesc":
    "El Departamento de Trabajo investigará a su empleador sin costo alguno. No necesita un abogado. Presente su queja en línea ahora.",
  "workers.unemployment": "Solicite beneficios por desempleo",
  "workers.unemploymentDesc":
    "Si perdió su empleo, es probable que califique. Todos los estados cuentan con un formulario de solicitud en línea. Usted ha contribuido a este sistema; estos son beneficios que se ha ganado.",
  "workers.phone": "1-866-4-USWAGE (1-866-487-2365)",
  "workers.phoneDesc":
    "Departamento de Trabajo: infracciones salariales y de horarios",
  // Food
  "food.title": "Alimentos",
  "food.cardDesc":
    "Cupones de alimentos, bancos de alimentos, comidas escolares, línea de ayuda contra el hambre",
  "food.pageDesc":
    "Asistencia alimentaria, beneficios SNAP, bancos de alimentos y comidas escolares gratuitas.",
  "food.snapEligibility":
    "Es posible que usted ya califique para recibir asistencia alimentaria",
  "food.snapEligibilityDesc":
    "Millones de estadounidenses califican para SNAP (cupones de alimentos), pero nunca presentan la solicitud. Si tiene dificultades para costear sus alimentos, podría ser elegible para recibir beneficios mensuales, sin importar su edad ni el tamaño de su familia.",
  "food.schoolMeals": "Sus hijos pueden comer gratis en la escuela",
  "food.schoolMealsDesc":
    "Si su familia tiene ingresos bajos, sus hijos pueden recibir desayuno y almuerzo gratuitos todos los días escolares. Este es un programa federal; todas las escuelas públicas de Estados Unidos participan en él.",
  "food.foodBank": "Encuentre alimentos gratuitos cerca de usted",
  "food.foodBankDesc":
    "Los bancos de alimentos distribuyen víveres gratuitos: sin necesidad de identificación, sin papeleo y sin hacer preguntas. Ingrese su código postal para encontrar uno cerca de usted ahora mismo.",
  "food.snapApply": "Solicite SNAP (Cupones de alimentos)",
  "food.snapApplyDesc":
    "SNAP deposita dinero en una tarjeta cada mes para la compra de alimentos. Puede presentar su solicitud en línea a través del sitio web de su estado. Si cumple con los requisitos, los beneficios pueden comenzar en un plazo de 30 días.",
  "food.phone": "1-866-3-HUNGRY (1-866-348-6479)",
  "food.phoneDesc":
    "Línea Nacional de Ayuda contra el Hambre del USDA --- ayuda gratuita para encontrar alimentos cerca de usted",
  // Donate
  "donate.title": "Done",
  "donate.message":
    "Done a The General Welfare Project votando en las elecciones de mitad de mandato de 2026",
} as Record<string, string>;
